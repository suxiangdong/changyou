...
type config struct {
	...
	AddProperties map[string]string   `mapstructure:"properties"`
...
type propertiesSetting struct {
	...
	AddProperties map[string]string      `mapstructure:"properties"`
	MergeMap      []*MergeToObjectArray  `mapstructure:"merge_map"`
}
...
func (s *propertiesSetting) parseProperties() {
	if s.AddProperties == nil {
		s.AddProperties = cfg.AddProperties
	}
}
